...
const judgeNode: GraphNode<typeof state> = async (state) => {
  ...
  const judgeResponse = await judgeAI.invoke({
    messages: [
      new HumanMessage(`
            Problem: ${problem},
            Solution 1: ${solution_1},
            Solution 2: ${solution_2}
            Please evaluate the solutions and provide sores and reasoning.
            `),
    ],
  });
...
export default async function runGraph(problem: string): Promise<object> {
